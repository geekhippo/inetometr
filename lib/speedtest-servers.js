...
export const SERVERS = [
  {
    ...
    region: 'RU',
    // Наш production на 87.121.221.180 (Амстердам, РФ-сегмент)
    // Файл speedtest/50mb.bin кладётся в /var/www/inetometr/speedtest/.
    // В проде использует same-origin (CORS не нужен),
    // в локальной разработке тоже same-origin (127.0.0.1:8766).
    downloadUrl: '/speedtest/50mb.bin',
    uploadUrl: null,  // нет своего upload-handler'а → используем CF
    pingUrl: '/speedtest/50mb.bin?ping&_={n}',
    sizeBytes: 52428800,
    priority: 1,
  },
  {
