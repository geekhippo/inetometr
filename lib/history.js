...
export function clearHistory() {
  if (typeof localStorage === 'undefined') return;
  try { localStorage.removeItem(STORAGE_KEY); } catch (_) { /* quota or disabled */ }
}
...
export function groupByDay(history, days = 7) {
  ...
  const start = startOfToday - ((days - 1) * dayMs);
  /** @type {Map<number, {download: number[], upload: number[], count: number}>} */
  const out = new Map();
  for (let i = 0; i < days; i++) {
    out.set(start + i * dayMs, { download: [], upload: [], count: 0 });
  }
  for (const e of history) {
    const dayStart = e.t - (e.t % dayMs);
    if (dayStart < start || dayStart >= start + days * dayMs) continue;
