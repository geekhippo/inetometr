...
export async function fetchServerMeta(url = 'https://speed.cloudflare.com/__down?bytes=1024') {
  try {
    ...
    // Fallback: cf-meta-* может не прокидываться при проксировании через
    // Cloudflare (Free-план). Дополняем через /cdn-cgi/trace, который
    // гарантированно возвращает colo/идентификатор дата-центра.
    if (!meta.colo) {
      try {
 ...
 * Пример: "Москва, RU (MOW)" или "Cloudflare · MOW" если есть только colo.
...
export function describeServer(meta) {
  ...
  if (meta.colo) {
    if (parts.length > 0) {
      // Сначала город/страна, потом идентификатор дата-центра
      parts.push(`CDN ${meta.colo}`);
    } else {
      // Только colo — добавляем «Cloudflare» для понятности
